<script>
    ...
    async function handleSubmit(){
        ...
        console.log("TEST : " + JSON.stringify(selectedProduct));
        resetForm();
        await invalidateAll();
    }
...
<main class="p-2">
    <form class="boder py-6 px-2" on:submit|preventDefault={handleSubmit}>
        <div class="flex flex-col">
            ...
            <input id="name" type="text" class="py-1 px-2 border text-black" bind:value={selectedProduct.name} />
        </div>
        <div class="flex flex-col">
            <label for="description">Description</label>
            <textarea id="description" class="py-1 px-2 border text-black" placeholder="optional" bind:value={selectedProduct.description}></textarea>
        </div>
        <div class="flex flex-col">
            <label for="price">Price</label>
            <input id="price" type="text" class="py-1 px-2 border text-black" bind:value={selectedProduct.price} />
        </div>
        <div class="flex flex-col">
            <label for="imageUrl">Image URL</label>
            <input id="imageUrl" type="text" class="py-1 px-2 border text-black" bind:value={selectedProduct.imageUrl} />
        </div>
        <div class="mt-6">
